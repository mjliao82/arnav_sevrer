...
# Create resizer
resizer_size = 80
resizer_img = pygame.image.load("resizer.jpg").convert()
resizer = pygame.transform.scale(resizer_img, (resizer_size, resizer_size))
resizer_rect = resizer.get_rect(topleft=(100, 10))
...
while running:
    ...
    if (player.x > platform_1_x and player.x < platform_1_x + platform_1_width):
        if player.y < platform_1_y and (player_velocity < 0):
            if player.colliderect(platform_1):
                player.y = ground.y - player_size
                player_on_ground = True
                player_velocity = 0
    ...
    if player.colliderect(resizer_rect) and not small:
        player_x = 100
        player_y = 270
        player_size = 10
        player_rect = pygame.transform.scale(player_img, (player_size, player_size))
        ...
        small = True
    elif player.colliderect(resizer_rect) and small:
        player_x = 100
        player_y = 270
        player_size = 40
        player_rect = pygame.transform.scale(player_img, (player_size, player_size))
